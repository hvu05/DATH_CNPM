...
export const createReview = async (
  data: reviewDto.ReviewCreateRequest,
  userId: string,
  productId: number,
): Promise<reviewDto.ReviewResponse> => {
  ...
  if (!order) {
    throw new AppError(
      ErrorCode.NOT_FOUND,
      'Người dùng cần phải mua hàng thì mới đánh giá được sản phẩm này!',
    );
  }
...
export const getAllReviewsOfProduct = async (
  productId: number,
  query: reviewDto.ReviewListQuery,
): Promise<reviewDto.ReviewListResponse> => {
