...
import type {
    ...
    WardRequest,
    WardsResponse,
} from '@/types/clients/client.address.types';
...
export const addressAPI = {
    ...
    getAwards: async (params: WardRequest): Promise<WardsResponse> => {
        const res = await fetch(
            `https://production.cas.so/address-kit/2025-07-01/provinces/${params.codeProvince}/communes`,
            { method: 'GET' }
        );
        if (!res.ok) {
    ...
    getAddress: async () => {
        const res = await axios.get<AddressResponse>(`/users/address`);
