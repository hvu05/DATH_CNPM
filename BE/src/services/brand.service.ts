...
import { BrandCreateRequest, BrandUpdateRequest } from "../dtos/brand/brand.-create.resquest";

export const brandService = {
  async createBrand(data: BrandCreateRequest) {
    const existing = await prisma.brand.findUnique({ where: { name: data.name } });
    if (existing) throw new AppError(ErrorCode.CONFLICT, `Brand "${data.name}" already exists`);
  ...
  async getBrandById(id: number | string) {
    const brandId = typeof id === "string" ? parseInt(id) : id;
    const brand = await prisma.brand.findUnique({ where: { id: brandId }, include: { category: true } });
