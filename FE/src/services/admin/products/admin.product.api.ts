...
export const getAllCategoriesAPI = async (params: IGetCategoriesParam = {}) => {
    ...
    const result = await axios.get<ApiResponse<ICategoriesResponse>>(`${import.meta.env.VITE_BACKEND_URL}/admin/categories?${queryParams.toString()}`);

    return result.data;
}

export const getBrandsAPI = async () => {
    const result = await axios.get<ApiResponse<IBrandsResponse>>(`${import.meta.env.VITE_BACKEND_URL}/admin/brands`);
    return result.data;
}

export const getSeriesAPI = async () => {
    const result = await axios.get<ApiResponse<ISeriesRes>>(`${import.meta.env.VITE_BACKEND_URL}/admin/series`);
    return result.data;
}
