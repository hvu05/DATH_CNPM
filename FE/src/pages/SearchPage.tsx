...
// --- DỮ LIỆU BỘ LỌC CỐ ĐỊNH (HARDCODE) ---
...
const SearchPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            const data = await getProducts(); // Lấy tất cả
            setAllProducts(data);
            setLoading(false);
        };
        fetchData();
    ...
    // 2. Logic Lọc Client-side (Cực mạnh)
    useEffect(() => {
        ...
        // Lọc Hãng (So sánh tương đối để tránh lỗi hoa thường)
    ...
    // --- RENDERS NỘI DUNG POPOVER (Dùng dữ liệu Hardcode) ---
